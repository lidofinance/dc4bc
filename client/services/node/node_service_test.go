...
// func TestClient_GetOperationPath(t *testing.T) {
// 	var (
// 		ctx  = context.Background()
// 		req  = require.New(t)
// 		ctrl = gomock.NewController(t)
...
// 	var expectedQrPath = filepath.Join(jsonFilesDir, fmt.Sprintf("dc4bc_json_%s.json", operation.ID))
// 	defer os.Remove(expectedQrPath)

// 	state.EXPECT().GetOperationByID(operation.ID).Times(1).Return(
// 		nil, errors.New(""))
// 	_, err = clt.GetOperationPath(&dto.OperationIdDTO{OperationID: operation.ID})
// 	req.Error(err)
