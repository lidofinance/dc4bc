...
run-client-node:
	...
	@docker run -it -e STORAGE_TOPIC=foo -e STORAGE_DBDSN=$(STORAGE_DBDSN) -e STORAGE_TOPIC=$(STORAGE_TOPIC) -e USERNAME=$(USERNAME) --name client_node -v $(DATA_DIR):/go/src/shared -p client_node:latest
